...
$subTotal = get("SELECT SUM(B.price*C.quantity) 'total' FROM cart C RIGHT OUTER JOIN books B ON C.isbn=B.isbn WHERE email=\"$email\"")["total"];
$deliveryFee = 300;
$grandTotal = $deliveryFee + $subTotal;

function showOrderSummery(){
    global $email, $subTotal, $grandTotal, $deliveryFee;
    $orderItems = execute("SELECT B.name, B.price, C.quantity, B.price*C.quantity 'total' FROM cart C RIGHT OUTER JOIN books B ON C.isbn=B.isbn WHERE email=\"$email\"");
